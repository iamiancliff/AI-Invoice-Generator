...
const parseInvoiceFromText = async (req, res) => {
    ...
    res.status(200).json(parseData);
        ...
        res.status(500).json({ message: "AI processing failed. Please check your input and try again.", details: error.message });
    }
};
...
const generateReminderEmail = async (req, res) => {
    ...
    res.status(200).json({ emailContent: response.text });
        ...
        res.status(500).json({ message: "Request could not be processed at this time.", details: error.message });
    }
};
